...
pub struct CommonTheme {
    ...
    border_radius_floating: String,
    border_radius_circular: String,

    spacing_horizontal_none: String,
    ...
    spacing_horizontal_x_l: String,
    spacing_horizontal_x_x_l: String,
    spacing_horizontal_x_x_x_l: String,
    ...
    spacing_vertical_x_l: String,
    spacing_vertical_x_x_l: String,
    spacing_vertical_x_x_x_l: String,
...
impl CommonTheme {
    pub fn new() -> Self {
        Self {
            font_family_base:
                "'League Spartan', ui-sans-serif, system-ui, -apple-system, sans-serif".into(),
            font_family_monospace: "'League Mono', ui-monospace, monospace".into(),
            font_family_numeric: "'League Spartan', ui-sans-serif, system-ui, sans-serif".into(),
            font_family_display:
                "'Orbitron', 'League Spartan', ui-sans-serif, system-ui, sans-serif".into(),
            ...
            border_radius_floating: "16px".into(),
            border_radius_circular: "10000px".into(),

            spacing_horizontal_none: "0".into(),
            ...
            spacing_horizontal_x_l: "20px".into(),
            spacing_horizontal_x_x_l: "24px".into(),
            spacing_horizontal_x_x_x_l: "32px".into(),
            ...
            spacing_vertical_x_l: "20px".into(),
            spacing_vertical_x_x_l: "24px".into(),
            spacing_vertical_x_x_x_l: "32px".into(),
    ...
    pub fn write_orb_motion_css_vars(&self, css_vars: &mut String) {
        ...
        const CURVE_DECELERATE_MIN: &str = "cubic-bezier(0.33, 0, 0.1, 1)";

        let durations = [
            (
                "--orb-motion-duration-2xs",
                "--durationUltraFast",
                self.duration_ultra_fast.as_str(),
            ),
            (
                "--orb-motion-duration-xs",
                "--durationFaster",
                self.duration_faster.as_str(),
            ),
            ("--orb-motion-duration-sm", "--durationFast", DURATION_FAST),
            (
                "--orb-motion-duration-md",
                "--durationNormal",
                self.duration_normal.as_str(),
            ),
            (
                "--orb-motion-duration-lg",
                "--durationGentle",
                self.duration_gentle.as_str(),
            ),
            (
                "--orb-motion-duration-xl",
                "--durationSlow",
                self.duration_slow.as_str(),
            ),
            (
                "--orb-motion-duration-2xl",
                "--durationSlower",
                DURATION_SLOWER,
            ),
            (
        ...
        for (orb, legacy, value) in durations {
            ...
            css_vars.push_str(&format!("{legacy}: {value};"));
        }

        let curves = [
            ("--orb-motion-ease-linear", "--curveLinear", CURVE_LINEAR),
            (
                "--orb-motion-ease-accelerate-strong",
                "--curveAccelerateMax",
                CURVE_ACCELERATE_MAX,
            ),
            (
                "--orb-motion-ease-accelerate",
                "--curveAccelerateMid",
                self.curve_accelerate_mid.as_str(),
            ),
            (
                "--orb-motion-ease-accelerate-subtle",
                "--curveAccelerateMin",
                CURVE_ACCELERATE_MIN,
            ),
            (
                "--orb-motion-ease-decelerate-strong",
                "--curveDecelerateMax",
                self.curve_decelerate_max.as_str(),
            ),
            (
                "--orb-motion-ease-decelerate",
                "--curveDecelerateMid",
                self.curve_decelerate_mid.as_str(),
            ),
            (
                "--orb-motion-ease-decelerate-subtle",
                "--curveDecelerateMin",
                CURVE_DECELERATE_MIN,
            ),
            (
                "--orb-motion-ease-standard",
                "--curveEasyEase",
                self.curve_easy_ease.as_str(),
            ),
            (
                "--orb-motion-ease-emphasis",
                "--curveEasyEaseMax",
                self.curve_easy_ease_max.as_str(),
            ),
        ];

        for (orb, legacy, value) in curves {
            css_vars.push_str(&format!("{orb}: {value};"));
            css_vars.push_str(&format!("{legacy}: {value};"));
        }
    }
