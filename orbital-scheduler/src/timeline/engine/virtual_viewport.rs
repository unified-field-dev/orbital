//! Virtual row/column window math for bounded timeline scrollports.

/// Default number of extra rows rendered above/below the visible window.
pub const DEFAULT_ROW_OVERSCAN: usize = 3;
...
/// Build cumulative left offsets for columns.
...
pub fn compute_column_viewport(
    ...
    let offsets = column_left_offsets(column_widths);
